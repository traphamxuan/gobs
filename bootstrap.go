...
func (bs *Bootstrap) Init(ctx context.Context) error {
	...
	return bs.execute(ctx, common.StatusInit)
}

func (bs *Bootstrap) Setup(ctx context.Context) error {
	return bs.execute(ctx, common.StatusSetup)
}

func (bs *Bootstrap) Start(ctx context.Context) error {
	return bs.execute(ctx, common.StatusStart)
}

func (bs *Bootstrap) Stop(ctx context.Context) error {
	return bs.execute(ctx, common.StatusStop)
}
...
func (bs *Bootstrap) execute(ctx context.Context, ss common.ServiceStatus) (err error) {
	var tasks []types.ITask
	if ss > common.StatusInit {
		previousState := ss - 1
		bs.schedulers[previousState].Interrupt()
		tasks, err = bs.schedulers[previousState].Release()
		if err != nil {
			bs.Logger.Log("Previous state %s hash error %s", previousState.String(), err.Error())
		}
	} else {
		tasks = make([]types.ITask, 0, len(bs.services))
		for _, sb := range bs.services {
